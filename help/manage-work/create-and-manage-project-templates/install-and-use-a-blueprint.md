---
title: Installera och använda en utkast
description: Lär dig hur en administratör installerar en plan och hur en användare använder den.
activity: use
feature: Work Management
type: Tutorial
role: User, Admin
level: Beginner
team: Technical Marketing
last-substantial-update: 2026-2-12
recommendations: noDisplay,catalog
jira: KT-20314
doc-type: video
source-git-commit: bb1f0cb236ff68aaa89a6d6f11b5e3f4bba06dcc
workflow-type: tm+mt
source-wordcount: '172'
ht-degree: 0%

---

# Installera och använda en utkast

Lär dig hur en administratör installerar en plan och hur en användare använder den. I det här exemplet ska vi installera&quot;Workfront Training for New Users&quot;-planen och använda de mallar och vyer som den skapar som en checklista för utbildning för att underlätta för användare i Workfront.


>[!VIDEO](https://video.tv.adobe.com/v/3479838/?captions=swe&quality=12&learn=on&enablevpops=0)

## Viktiga uppgifter

* Så här väljer du en plan och avgör om den redan är installerad eller inte. Detta kan de flesta användare göra med en standardlicens.
* Så här installerar du en plan och delar objekt som skapats av den. Detta kräver en systemadministratörs tjänster.
* Så här söker du efter och använder nya projektmallar som skapats med en plan.


## Rekommenderade självstudiekurser i detta ämne

* [Skapa en projektmall och lär dig mer om utkast](/help/manage-work/create-and-manage-project-templates/create-a-project-template.md)
* [Skapa ett projekt direkt från en mall](/help/manage-work/create-and-manage-project-templates/create-a-project-directly-from-a-template.md)
* [Dela en projektmall](/help/manage-work/create-and-manage-project-templates/share-a-project-template.md)
* [Kopiera ett befintligt projekt](/help/manage-work/manage-projects/copy-an-existing-project.md)
* [Inaktivera en projektmall](/help/manage-work/create-and-manage-project-templates/deactivate-a-project-template.md)
* [Redigera projektteamet i en projektmall](/help/manage-work/create-and-manage-project-templates/edit-the-project-team-in-a-project-template.md)
